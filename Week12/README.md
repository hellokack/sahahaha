...
* **E2E 테스트 시나리오 설계 (Playwright):** * 사용자가 회원가입 폼에 진입하여 이메일을 입력하고 제출 버튼을 클릭하는 시나리오 작성.
  * 실패 시(예: 잘못된 이메일 형식 입력) `page.screenshot({ path: 'error.png' })`를 호출하여 스크린샷 아티팩트를 자동 저장하도록 구성.
* **리팩토링 안정성 검증:** * 레거시 코드에 테스트 코드를 선제적으로 보강한 후, 내부 로직(정규식 성능 개선 등)을 리팩토링함. 
  * 기존 단위 테스트가 모두 통과하는 것을 확인하여 기능의 회귀(Regression) 현상 없이 안정적으로 리팩토링을 완료함.
